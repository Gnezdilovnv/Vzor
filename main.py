...
def validate_frequency(v): return bool(re.fullmatch(r"\d+(\.\d+)?", v))
def validate_date(v):
...
def load_json(path, default=None):
    ...
    except Exception as e: log_error(f"load_json {path}: {e}")
    return default

def save_json(path, data):
    try:
        with open(path, "w") as f: json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e: log_error(f"save_json {path}: {e}")
...
def update_record(rid, new_data):
    records = load_records()
    for r in records:
        if r["id"] == rid: r.update(new_data); break
    save_records(records)
...
def export_pdf(records, path):
    if not PDF_AVAILABLE: raise RuntimeError("fpdf missing")
    pdf = FPDF(); pdf.add_page(); pdf.set_font("Arial", size=10)
    pdf.cell(200, 10, "Vzor Report", ln=True, align='C'); pdf.ln(5)
    cols = [("Date",25),("Time",15),("Direction",30),("Point",25),("Type",20),("Freq v",25),("Freq c",25),("Suppr",20)]
    for h,w in cols: pdf.cell(w,7,h,border=1)
    pdf.ln()
    for r in records:
        pdf.cell(25,6,r["date"],border=1); pdf.cell(15,6,r["time"],border=1)
        pdf.cell(30,6,r["direction"],border=1); pdf.cell(25,6,r["point"],border=1)
        pdf.cell(20,6,r["type"],border=1); pdf.cell(25,6,r["freq_video"],border=1)
        pdf.cell(25,6,r["freq_control"],border=1); pdf.cell(20,6,"Yes" if r["suppressed"] else "No",border=1)
        pdf.ln()
    pdf.output(path)
...
def main(page: ft.Page):
    try:
        ...
        def dd(label, hint, opts, val, expand=True):
            return ft.Dropdown(label=label, hint_text=hint,
        ...
        def save_settings(e):
            ...
            page.snack_bar.open = True; page.update(); page.go("/")
        ...
        def home_view():
            ...
            def on_type_change(e):
                if type_dd.value in TYPE_FREQ_PRESETS:
                    fv.value, fc.value = TYPE_FREQ_PRESETS[type_dd.value]; page.update()
            ...
            def apply_tmpl(name):
                ...
                if t:
                    type_dd.value = t.get("type"); fv.value = t.get("freq_video","")
                    fc.value = t.get("freq_control",""); supp_sw.value = t.get("suppressed",False)
                    page.update()
            ...
            def save_tmpl(e):
                ...
                if not name:
                    page.snack_bar = ft.SnackBar(ft.Text("Enter name")); page.snack_bar.open = True; page.update(); return
                settings.setdefault("templates",{})[name] = {
                    "type": type_dd.value, "freq_video": fv.value,
                    "freq_control": fc.value, "suppressed": supp_sw.value}
                save_json(SETTINGS_FILE, settings); refresh_templates()
            ...
            def load_page(off=0, clear=True):
                nonlocal offset
                if clear:
                    rec_list.controls.clear(); offset = 0
                filters = get_filters()
            ...
            def edit_dialog(rid):
                ...
                def save_edit(e):
                    ...
                    dlg.open = False; load_page(); page.update()
            ...
            def on_save(e):
                ...
                if not settings.get("direction") or not settings.get("point"):
                    page.snack_bar = ft.SnackBar(ft.Text("Set direction/point in settings")); page.snack_bar.open = True; page.update(); return
                if not type_dd.value:
                    page.snack_bar = ft.SnackBar(ft.Text("Select type!")); page.snack_bar.open = True; page.update(); return
                if not fv.value or not fc.value:
                    page.snack_bar = ft.SnackBar(ft.Text("Fill frequencies!")); page.snack_bar.open = True; page.update(); return
                if not validate_frequency(fv.value) or not validate_frequency(fc.value):
                    page.snack_bar = ft.SnackBar(ft.Text("Invalid frequency")); page.snack_bar.open = True; page.update(); return
                if not validate_date(date_f.value) or not validate_time(time_f.value):
                    page.snack_bar = ft.SnackBar(ft.Text("Invalid date/time")); page.snack_bar.open = True; page.update(); return
                ...
                add_record(data); load_page()
                fv.value = ""; fc.value = ""; supp_sw.value = False
                date_f.value = datetime.now().strftime("%Y-%m-%d"); time_f.value = datetime.now().strftime("%H:%M")
            ...
            def export_view(fmt):
                ...
                if not records:
                    page.snack_bar = ft.SnackBar(ft.Text("No records")); page.snack_bar.open = True; page.update(); return
                os.makedirs(REPORTS_DIR, exist_ok=True)
                ts = datetime.now().strftime('%Y%m%d_%H%M%S')
                if fmt=="csv":
                    path = os.path.join(REPORTS_DIR, f"export_{ts}.csv"); export_csv(records, path)
                elif fmt=="pdf":
                    if not PDF_AVAILABLE:
                        page.snack_bar = ft.SnackBar(ft.Text("Install fpdf2")); page.snack_bar.open = True; page.update(); return
                    path = os.path.join(REPORTS_DIR, f"export_{ts}.pdf"); export_pdf(records, path)
                page.snack_bar = ft.SnackBar(ft.Text(f"Exported to {path}"), bgcolor=ft.Colors.GREEN)
                page.snack_bar.open = True; page.update()

            def do_backup(e):
                path = create_backup()
                page.snack_bar = ft.SnackBar(ft.Text(f"Backup: {path}")); page.snack_bar.open = True; page.update()

            def do_restore(e):
                fp = ft.FilePicker(on_result=lambda r: restore_and_refresh(r) if r.files else None)
                page.overlay.append(fp); fp.pick_files(allowed_extensions=["zip"])

            def restore_and_refresh(e):
                if e.files:
                    restore_backup(e.files[0].path); load_page()
                    page.snack_bar = ft.SnackBar(ft.Text("Restored")); page.snack_bar.open = True; page.update()

            def share_log(e):
                if os.path.exists(LOG_FILE):
                    tmp = f"share_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                    shutil.copy(LOG_FILE, tmp); page.launch_url(f"file://{os.path.abspath(tmp)}")
                    page.snack_bar = ft.SnackBar(ft.Text("Log shared"))
                else: page.snack_bar = ft.SnackBar(ft.Text("No log"))
                page.snack_bar.open = True; page.update()
        ...
        def stats_view():
            records = load_records()
            total = len(records); supp = sum(1 for r in records if r["suppressed"])
            by_type = {}; by_dir = {}
            for r in records:
        ...
        def route_change(route):
            page.views.clear()
            if route == "/settings": page.views.append(settings_view())
            elif route == "/stats": page.views.append(stats_view())
            else: page.views.append(home_view())
            page.update()
        ...
        log_error(f"MAIN CRASH: {traceback.format_exc()}")
        page.update()
